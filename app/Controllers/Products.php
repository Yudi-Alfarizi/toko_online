...
class Products extends BaseController
{
    ...
    public function index()
    {
        ...
        if ($categorySlug = $this->request->getGet('category')) {
            ...
            if ($category) {
                $childIds = $this->categoryModel->getDescendantCategoryIds($category->id);
                $categoryIds = array_merge([$category->id], $childIds);

                $db = \Config\Database::connect();
                $builder = $db->table('product_categories pc');
                $builder->select('products.parent_id');
                $builder->join('products', 'products.id = pc.product_id');
                $builder->whereIn('pc.category_id', $categoryIds);
                $builder->where('products.parent_id IS NOT NULL');
                $builder->groupBy('products.parent_id');
                $parentIdsResult = $builder->get()->getResultArray();
                $parentIds = array_column($parentIdsResult, 'parent_id');

                $products = $products->groupStart()
                    ->whereIn('product_categories.category_id', $categoryIds);

                if (!empty($parentIds)) {
                    $products = $products->orWhereIn('products.id', $parentIds);
                }

                $products = $products->groupEnd();
            }
        }
