...
class Attributes extends BaseController
{
    ...
    public function index($attributeId = null)
    {
        if ($attributeId) {
            ...
            if (!$attribute) {
                $this->session->setFlashdata('errors', 'Attribute tidak valid');
    ...
    public function store()
    {
        ...
        if ($this->attributeModel->save($params)) {
            $this->session->setFlashdata('success', 'Attribute berhasil disimpan.');
            return redirect()->to('/admin/attributes');
        } else {
            $this->getAttributes();
    ...
    public function update($id)
    {
        ...
        if ($this->attributeModel->save($params)) {
            $this->session->setFlashdata('success', 'Attribute berhasil diperbarui!');
            return redirect()->to('/admin/attributes');
        } else {
            $this->getAttributes();
    ...
    public function destroy($id)
    {
        ...
        if ($this->attributeModel->delete($attribute->id)) {
            $this->session->setFlashdata('success', 'Attribute berhasil dihapus');
            return redirect()->to('/admin/attributes');
        } else {
            $this->session->setFlashdata('errors', 'Tidak dapat menghapus Attribute');
            return redirect()->to('/admin/attributes');
        }
    }
}
